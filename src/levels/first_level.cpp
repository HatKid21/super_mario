...
void FirstLevel::init_data() {
	...
	ui_factory->create_enemy({20, 5}, 3, 2,0.8f);
	ui_factory->create_enemy({25, 5}, 3, 2,0.1f);
	ui_factory->create_enemy({70, 15}, 3, 2,0.6f);
	ui_factory->create_enemy({80, 5}, 3, 2);
	ui_factory->create_enemy({125, 5}, 3, 2,0.1f);
	ui_factory->create_enemy({160, 5}, 3, 2,0.4f);
